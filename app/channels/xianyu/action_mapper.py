...
ChannelAction = Literal["answer", "clarify", "human_handoff", "ignore"]
...
def map_chat_response(payload: Mapping[str, Any]) -> MappedAction:
    ...
    channel contract names them ``answer``/``clarify``/``human_handoff``.
    Unknown or malformed model/API output always becomes a handoff.
    """

    action = _text(payload.get("action")).lower()
    next_step = _text(payload.get("next_step")).lower()
    answer = _text(payload.get("answer"))
    clarify = _text(payload.get("clarify_question")) or answer
    reason = _text(payload.get("reason")) or _text(payload.get("error")) or "需要卖家确认"
    ...
    if action in {"clarify", "clarification"} or next_step in {"clarify", "clarification"}:
        ...
        return MappedAction("human_handoff", reason="澄清问题为空")
